 ...
 * Template Name: 導入事例
 *
 * Cases page template for Rinascente corporate site.
 *
 * @package Rinascente
 */
...
$case_plain_summary  = static function( $value ) {
    return function_exists( 'rinascente_case_plain_summary' )
        ? rinascente_case_plain_summary( $value )
        : trim( preg_replace( '/\s+/u', ' ', wp_strip_all_tags( (string) $value, true ) ) );
};
...
$featured_challenge     = $featured_case ? $case_plain_summary( $featured_case['challenge'] ?? '' ) : '';
$featured_reason        = $featured_case ? $case_plain_summary( $featured_case['reason'] ?? '' ) : '';
$featured_ringi         = $featured_case ? $case_plain_summary( $featured_case['ringi_process'] ?? '' ) : '';
$featured_quote         = $featured_case ? $case_plain_summary( $featured_case['pullquote'] ?? '' ) : '';
      ...
      <div class="container">
        ...
        <div class="cases-featured-grid fade-up" style="display:grid;grid-template-columns:<?php echo ! empty( $featured_case['image_url'] ) ? '5fr 6fr' : '1fr'; ?>;gap:clamp(24px,4vw,48px);align-items:start;border:1px solid rgba(0,0,0,0.08);border-radius:var(--r-xl);padding:clamp(28px,4vw,52px);background:white;">
          ...
          <div style="overflow:hidden;border-radius:var(--r-lg);">
            <img src="<?php echo esc_url( $featured_case['image_url'] ); ?>" alt="<?php echo esc_attr( $featured_case['facility'] ); ?>" style="width:100%;height:auto;display:block;">
          </div>
          <?php endif; ?>
          <div class="cases-featured-copy">
            ...
            <div class="cases-metrics" style="display:grid;grid-template-columns:repeat(<?php echo count( $featured_metrics ); ?>,minmax(0,1fr));gap:0;background:rgba(0,104,183,0.04);border-left:3px solid #0068b7;border-radius:0 6px 6px 0;margin-bottom:24px;">
              <?php foreach ( $featured_metrics as $index => $metric ) : ?>
          ...
          <div>
            <div style="display:flex;gap:10px;align-items:center;margin-bottom:20px;">
              <span class="badge badge-blue">YUMEHO</span>
            </div>
            <h2 style="font-size:clamp(1.3rem,2.5vw,2rem);font-weight:700;line-height:1.35;margin-bottom:16px;color:var(--charcoal);">
              YUMEHO 側の導入事例を準備中です。
            </h2>
            <p style="font-size:0.95rem;color:var(--mid-gray);line-height:1.75;margin-bottom:24px;">
              導入事例は YUMEHO サイトで入力し、Rinascente の公開ページへ共通表示します。
            </p>
      ...
      <div class="container">
        <div class="section-header fade-up" style="display:flex;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;gap:16px;">
          ...
          <div style="display:flex;gap:8px;">
            <button class="filter-btn active" style="padding:6px 14px;border-radius:999px;border:1px solid rgba(0,0,0,0.15);font-size:0.78rem;font-weight:700;background:var(--black);color:white;cursor:pointer;">すべて</button>
            <button class="filter-btn" style="padding:6px 14px;border-radius:999px;border:1px solid rgba(0,0,0,0.15);font-size:0.78rem;font-weight:700;background:white;color:var(--mid-gray);cursor:pointer;">YUMEHO</button>
            <?php if ( $mica30_enabled ) : ?>
            <button class="filter-btn" style="padding:6px 14px;border-radius:999px;border:1px solid rgba(0,0,0,0.15);font-size:0.78rem;font-weight:700;background:white;color:var(--mid-gray);cursor:pointer;">MICA30</button>
            <?php endif; ?>
          </div>
        </div>
